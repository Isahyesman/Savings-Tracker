/* ===========================================================
   ...
   Backed by Supabase (Postgres) instead of Firestore. Every
   write/read is a plain HTTP request — no persistent streaming
   connection, which is what kept silently hanging on Firestore.
=========================================================== */

import { createClient } from 'https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm';

const SUPABASE_URL = 'https://axhkgcvtuxmjilksisse.supabase.co';
const SUPABASE_ANON_KEY = 'sb_publishable_N4g0IgvpJ48fTFA-BuLI-w_1S9e_zZM';
const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);

/* ---------- Table + field-name mapping (JS camelCase <-> Postgres snake_case) ----------
   Keeps every function below working with the same camelCase
   property names as before, so almost nothing else in this file
   needs to change. */
const TABLES = {
  clients: 'clients',
...
const FIELD_MAP = {
  clients: { idNumber: 'id_number' },
  savingsEntries: { clientId: 'client_id', adminName: 'admin_name' },
  loans: { clientId: 'client_id', dueDate: 'due_date', dateIssued: 'date_issued', adminName: 'admin_name' },
  loanPayments: { loanId: 'loan_id', adminName: 'admin_name' },
  messages: { clientId: 'client_id' }
};
function toDbRow(tableKey, obj){
  const map = FIELD_MAP[tableKey] || {};
  const out = {};
  for(const [k,v] of Object.entries(obj)) out[map[k] || k] = v;
  return out;
}
function fromDbRow(tableKey, row){
  const map = FIELD_MAP[tableKey] || {};
  const reverse = {};
  for(const [k,v] of Object.entries(map)) reverse[v] = k;
  const out = {};
  for(const [k,v] of Object.entries(row)) out[reverse[k] || k] = v;
  // Postgres returns numeric foreign keys as JS numbers, but the rest of
  // the app compares IDs as strings (e.g. session.clientId). Stringify
  // every ID-like field so === comparisons work consistently everywhere.
  out.id = String(out.id);
  if(out.clientId !== undefined && out.clientId !== null) out.clientId = String(out.clientId);
  if(out.loanId !== undefined && out.loanId !== null) out.loanId = String(out.loanId);
  return out;
}
async function dbInsert(tableKey, obj){
  const { data, error } = await supabase.from(TABLES[tableKey]).insert(toDbRow(tableKey, obj)).select().single();
  if(error) throw error;
  return fromDbRow(tableKey, data);
}
async function dbUpdate(tableKey, id, patch){
  const { error } = await supabase.from(TABLES[tableKey]).update(toDbRow(tableKey, patch)).eq('id', id);
  if(error) throw error;
}
async function dbDelete(tableKey, id){
  const { error } = await supabase.from(TABLES[tableKey]).delete().eq('id', id);
  if(error) throw error;
}
async function dbSelectAll(tableKey){
  const { data, error } = await supabase.from(TABLES[tableKey]).select('*');
  if(error) throw error;
  return data.map(row => fromDbRow(tableKey, row));
}
async function dbSelectWhere(tableKey, filtersCamelCase){
  const dbFilters = toDbRow(tableKey, filtersCamelCase);
  let q = supabase.from(TABLES[tableKey]).select('*');
  for(const [col, val] of Object.entries(dbFilters)) q = q.eq(col, val);
  const { data, error } = await q;
  if(error) throw error;
  return data.map(row => fromDbRow(tableKey, row));
}
async function dbGetById(tableKey, id){
  const { data, error } = await supabase.from(TABLES[tableKey]).select('*').eq('id', id).maybeSingle();
  if(error) throw error;
  return data ? fromDbRow(tableKey, data) : null;
}
...
const APP_BUILD = '2026.07.26-13-balancefix';
...
function clientMessages(clientId){
  return state.messages.filter(m => m.clientId === clientId).sort((a,b)=> a.date.localeCompare(b.date) || (Number(a.id) - Number(b.id)));
}
...
/* ---------- Live sync (Admin) — polling instead of a streaming connection ---------- */
...
async function refreshAdminData(){
  try{
    const [clients, savingsEntries, loans, loanPayments, messages] = await Promise.all([
      dbSelectAll('clients'), dbSelectAll('savingsEntries'), dbSelectAll('loans'),
      dbSelectAll('loanPayments'), dbSelectAll('messages')
    ]);
    state.clients = clients;
    state.savingsEntries = savingsEntries;
    state.loans = loans;
    state.loanPayments = loanPayments;
...
function subscribeAdminData(){
  refreshAdminData();
  adminPollTimer = setInterval(refreshAdminData, 4000);
}
...
function confirmLoan(){
  ...
  dbInsert('loans', { clientId, principal, rate, dueDate: due, status:'active', dateIssued: todayStr(), adminName }).then(()=>{
    document.getElementById('loanReview').classList.remove('show');
    document.getElementById('loanConfirmRow').style.display='none';
...
function confirmPayment(loanId){
  ...
  dbInsert('loanPayments', { loanId, amount, date: todayStr(), adminName }).then(async ()=>{
    if(newBal<=0){
      await dbUpdate('loans', loanId, { status: 'paid' }).catch(err=>console.error(err));
    }
...
function saveClientEdit(){
  ...
  dbUpdate('clients', clientId, { name, idNumber, phone, username, password }).then(()=>{
...
function deleteClientNow(){
  ...
  dbDelete('clients', clientId).catch(err=>{
    console.error(err);
...
async function openAdminThread(clientId){
  ...
  for(const m of unread){
    try{ await dbUpdate('messages', m.id, { read: true }); }catch(err){ console.error(err); }
  }
}
...
function sendAdminReply(){
  ...
  dbInsert('messages', { clientId: uiState.adminThreadClientId, sender:'admin', text, date: todayStr(), read:true }).catch(err=>{
    console.error(err);
...
async function createAccount(){
  ...
  // no extra network round-trip needed, since the dashboard keeps
  // state.clients refreshed automatically every few seconds.
  ...
  submitBtn.disabled = true; submitBtn.textContent = 'Saving...';
  try{
    await withTimeout(dbInsert('clients', { username, idNumber, password, name, phone }));
...
function initClientLogin(){
  ...
  form.addEventListener('submit', async function(e){
    ...
    try{
      const matches = await dbSelectWhere('clients', { username, idNumber, password });
      if(matches.length===0){
        ...
        // the mismatch directly without needing the Supabase dashboard.
        let diagnostic = '';
        try{
          const idOnlyMatches = await dbSelectWhere('clients', { idNumber });
          if(idOnlyMatches.length===0){
            diagnostic = `No client found with ID number "${idNumber}" at all.`;
          }else{
            const found = idOnlyMatches[0];
      ...
      errorBox.classList.remove('show');
      setSession({ adminId: null, clientId: matches[0].id });
...
async function refreshClientData(clientId){
  try{
    const [savingsEntries, loans, loanPayments, messages] = await Promise.all([
      dbSelectWhere('savingsEntries', { clientId }),
      dbSelectWhere('loans', { clientId }),
      dbSelectAll('loanPayments'),
      dbSelectWhere('messages', { clientId })
    ]);
    state.savingsEntries = savingsEntries;
    state.loans = loans;
    state.loanPayments = loanPayments;
...
function subscribeClientData(clientId){
  ...
  clientPollTimer = setInterval(()=>refreshClientData(clientId), 4000);
}
...
function renderClientDashboard(){
  ...
  if(!client){
    // client row not loaded into state yet — fetch it once directly
    dbGetById('clients', session.clientId).then(row=>{
      if(row){
        state.clients = [row];
        renderClientDashboard();
      }
...
function sendClientMessage(){
  ...
  if(!text) return;
  const session = getSession();
  dbInsert('messages', { clientId: session.clientId, sender:'client', text, date: todayStr(), read:false }).catch(err=>{
    console.error(err);
    alert('This message could not be sent permanently — please check your connection.');
  });
